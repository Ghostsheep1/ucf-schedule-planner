<script lang="ts">
	...
	function exportCalender() {
			...
			for (const packet of meetingDetails) {
				if (!packet.days || !packet.start || !packet.end) {
					...
					if (locLower.includes('online') || locLower.includes('async')) {
						...
						icsData += `UID:${courseCode}-${sectionNumber}-online@knightplanner\r\n`;
						icsData += 'END:VEVENT\r\n';
					}
					continue;
				}
				...
				icsData += `UID:${courseCode}-${sectionNumber}-${packet.start || ''}@knightplanner\r\n`;
				icsData += 'END:VEVENT\r\n';
			}
		}
		...
		a.href = url;
		a.download = 'Knight_Planner_Schedule.ics';
